...
import getMenuData from '../menuUtil';
...
const DiningScreen = ({data, theme}: DiningProps) => {
  ...
  return (
    <View
      style={[styles.fullScreen, {backgroundColor: theme.BACKGROUND_COLOR}]}>
      <FlatList
        data={data}
        ...
        renderItem={({item}) => (
          <View
            ...
            <Text
              style={[styles.restaurantTitle, {color: theme.TEXT_COLOR_BLACK}]}>
              {item.name}
            </Text>
            {item.meals.length > 0 ? (
                ...
                <View key={index} style={styles.mealContainer}>
                  ...
                  {meal.dishes.length > 0 ? (
                    meal.dishes.map((dish, idx) => (
                      <Text
                        key={idx}
                        style={[styles.dish, {color: theme.TEXT_COLOR}]}>
                        {dish}
                      </Text>
                    ))
                  ) : (
                    <Text
                      style={[styles.message, {color: theme.TEXT_COLOR_LIGHT}]}>
                      메뉴가 준비되지 않았어요
                    </Text>
                  )}
                </View>
        ...
        <SafeAreaProvider>
          <View style={styles.rootContainer}>
            ...
            <SafeAreaView style={styles.content}>
              ...
              <BannerAd
                unitId="ca-app-pub-9011101777574758/4163481682" // Replace with your AdMob unit ID
